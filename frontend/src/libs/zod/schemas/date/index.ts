...
export const zodDateStringSchema = z
  ...
  .refine((dateString) => !!dateString.match(/^([0-9]){2}\/([0-9]){2}\/([0-9]){4}$/), {
    message: "A data deve estar no formato dd/mm/aaaa, como por exemplo, 21/11/2024."
  })
  .refine(
    (dateString) => {
    ...
    { message: "Ops! Você digitou uma data inválida. Confira e tente novamente." }
  );
